...
import { llmJsonCompletion, llmChatCompletion } from '@/lib/llm-client'
import { validateLLMConfig } from '@/lib/llm-config'
import { logger } from '@/lib/logger'

const log = logger.module('generate-daily')
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    log.info('收到日报生成请求', { date: targetDate })
    ...
    const { data: existingDoc } = await db
      ...
      .select('*')
      .eq('user_id', userId)
      .eq('doc_date', targetDate)
      .single()
    
    if (existingDoc) {
      log.info('日报已存在，跳过生成', { docId: existingDoc.id })
    ...
    // 查询该日期收集的术语
    log.info('查询收集的术语', { userId, date: targetDate })
    ...
    if (!terms || terms.length === 0) {
      log.warn('没有找到收集的术语', { userId, date: targetDate })
      return NextResponse.json({
        message: '没有收集到术语，无法生成日报',
        doc_date: targetDate,
        terms_count: 0
      })
    }
    
    log.info('找到收集的术语', { count: terms.length })
    
    // 生成解释
    const llmConfig = validateLLMConfig()
    const useMock = isMockMode || !llmConfig.valid
    
    log.info('开始生成术语解释', { useMock, termsCount: terms.length })
    ...
    for (const term of terms) {
      ...
      if (useMock) {
        ...
        try {
          ...
          log.error(`生成术语解释失败: ${term.term}`, error)
          explanation = generateMockExplanation(term.term, term.domain)
        }
      }
    ...
    // 保存日报
    log.info('保存日报到数据库', { userId, date: targetDate, cardsCount: cards.length })
    
    const { data: savedDoc, error: saveError } = await db
      .from('daily_docs')
      .insert({
        user_id: userId,
        doc_date: targetDate,
    ...
    return NextResponse.json({
      ...
      doc_date: targetDate,
      term_count: cards.length,
      generated_at: savedDoc.generated_at
    })
    
  } catch (error) {
    log.error('日报生成错误', error)
    return NextResponse.json(
      { error: '日报生成失败' },
      { status: 500 }
    )
