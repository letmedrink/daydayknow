...
from ..llm import call_llm, get_llm_config, stream_llm
...
class ChatAgent:
    ...
    async def chat_stream(self, message: str, conversation_id: str, history: list[dict]):
        ...
        yield {
            "type": "_complete",
            "assistant_content": stored_clean,
            "references": references,
            "options": options if options else PRESET_OPTIONS,
        }
    ...
    def _retrieve_wiki_context(self, query: str) -> tuple[str, list[dict]]:
        ...
        """
        budget = compute_context_budget(get_llm_config(self.global_store).get("context_window", 0))
        retrieval = self.global_store.get_settings().get("retrievalConfig", {})
        candidate_limit = max(5, min(int(retrieval.get("candidateLimit", 12)), 30))
        rerank_limit = max(1, min(int(retrieval.get("rerankLimit", 5)), 10))
        if retrieval.get("mode") == "hybrid":
            search_results = self.wiki_store.hybrid_search(query, max_results=candidate_limit)[:rerank_limit]
        else:
            search_results = self.wiki_store.search(query, max_results=min(candidate_limit, 5))
        ...
        page_lookup = self.wiki_store.page_lookup()
        # 第一轮：直接匹配的页面（完整注入）
        for r in search_results:
            ...
            for link in extract_wikilinks(body)[:3]:
                np = page_lookup.get(link.strip().lower())
                if np and np["path"] not in visited and total < budget.page_budget:
                    visited.add(np["path"])
    ...
    def _trim_history(self, history: list[dict]) -> list[dict]:
        """Keep recent history within both message and character budgets."""
        config = get_llm_config(self.global_store)
        budget = compute_context_budget(config.get("context_window", 0))
        char_limit = max(2_000, budget.max_ctx - budget.page_budget - budget.response_reserve)
        selected = []
        total = 0
        for message in reversed(history[-20:]):
            content = str(message.get("content", ""))
            if selected and total + len(content) > char_limit:
                break
