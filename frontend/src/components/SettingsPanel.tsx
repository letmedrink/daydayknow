...
export function SettingsPanel() {
  ...
  const handleUpdateProvider = (id: string, field: string, value: any) => {
    setProviders((prev) => ({
      ...
      [id]: {
        ...prev[id],
        [field]: value,
        ...(field === 'api_key' ? { clear_api_key: false, has_api_key: Boolean(value) || prev[id].has_api_key } : {}),
      },
    }));
  };
  ...
  const handleClearKey = (id: string) => {
    setProviders((prev) => ({
      ...prev,
      [id]: { ...prev[id], api_key: '', has_api_key: false, clear_api_key: true },
    }));
  };
  ...
  return (
    <div style={styles.container}>
              ...
              <div style={styles.fieldGrid}>
                <div style={styles.field}>
                  ...
                  <input
                    ...
                    placeholder={provider.has_api_key ? '已保存；留空表示保持不变' : 'sk-...'}
                  />
                  {provider.has_api_key && !provider.clear_api_key && (
                    <button style={styles.testBtn} onClick={() => handleClearKey(id)}>清除已保存密钥</button>
                  )}
                </div>
                <div style={styles.field}>
                ...
                <div style={styles.field}>
                  <label style={styles.label}>上下文窗口（字符）</label>
                  <input
                    style={styles.input}
                    type="number"
                    min="4096"
                    value={provider.context_window || 32768}
                    onChange={(e) => handleUpdateProvider(id, 'context_window', parseInt(e.target.value))}
                  />
                </div>
              </div>
            )}
      ...
      <div style={styles.section}>🔎 Wiki 检索</div>
      <p style={styles.desc}>混合模式在本地合并倒排词法分数和字符模糊相似度，能容忍部分拼写和表达差异，不会额外向模型服务发送 Wiki 候选内容。</p>
      <select style={styles.select} value={retrievalMode} onChange={(e) => setRetrievalMode(e.target.value as 'lexical' | 'hybrid')}>
        <option value="lexical">词法检索（快速）</option>
        <option value="hybrid">词法 + 本地模糊重排</option>
      </select>

      <div style={styles.section}>🌐 Deep Research 搜索</div>
      <p style={styles.desc}>搜索摘要会发送给当前 LLM 用于综合；研究结果必须人工接受后才写入 Wiki。</p>
      <div style={styles.fieldGrid}>
        <div style={styles.field}>
          <label style={styles.label}>搜索 Provider</label>
          <select style={styles.select} value={searchProvider} onChange={(e) => setSearchProvider(e.target.value)}>
        ...
        <div style={styles.field}>
          <label style={styles.label}>API Key {searchHasKey && !clearSearchKey ? '（已保存）' : ''}</label>
          <input style={styles.input} type="password" value={searchApiKey} onChange={(e) => { setSearchApiKey(e.target.value); setClearSearchKey(false); }} placeholder={searchHasKey ? '留空保留现有 Key' : '输入搜索 API Key'} />
