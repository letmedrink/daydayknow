...
export function PreviewPanel() {
  ...
  const loadPages = useCallback(() => {
    import('../lib/api').then((api) => {
      api.fetchWikiPages(activeProjectId).then((data) => {
          ...
          {segments.map((seg, i) =>
            seg.type === 'text' ? (
              <ReactMarkdown
                ...
                components={{
                  img: ({ src, alt, ...props }) => {
                    // 相对路径 media/xxx → API 端点 /api/wiki/media/xxx
                    let imgSrc = src || '';
                    if (imgSrc.startsWith('media/') || imgSrc.startsWith('./media/')) {
                      imgSrc = `/api/wiki/media/${imgSrc.replace(/^\.?\/?media\//, '')}?project_id=${activeProjectId}`;
                    }
